...
class WMD(object):
	...
	def __init__(self, name):
		...
		#mapping commands
		self.bowtie2 = ['bowtie2','-p',str(args.threads),'--very-sensitive-local','-x',refName,self.name+'.fastq','-S',self.name+'.bowtie2.sam']
		self.bowtie2pe = ['bowtie2','-p',str(args.threads),'--very-sensitive-local','-x',refName,'-1',self.first+'.fastq','-2',self.second+'.fastq','-S',self.name+'.bowtie2.sam']
